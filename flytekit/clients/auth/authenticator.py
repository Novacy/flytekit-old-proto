...
class CommandAuthenticator(Authenticator):
    ...
    def refresh_credentials(self):
        ...
        """
        logging.debug(
            f"Starting external process to generate id token. Command {self._cmd}"
        )
        try:
            output = subprocess.run(self._cmd, capture_output=True, text=True, check=True)
        except subprocess.CalledProcessError as e:
            logging.error(f"Failed to generate token from command {self._cmd}")
            raise AuthenticationError(f"Problems refreshing token with command: {str(e)}")
...
class ClientCredentialsAuthenticator(Authenticator):
    ...
    def refresh_credentials(self):
        ...
        logging.info(f"Retrieved new token, expires in {expires_in}")
